...
def main():
    ...
    if not exchange.is_exchange_open():
        # Script cannot go any further from this point, but should not error.
        set_way_points(strategy.name, signals, '-', strategy.portfolio.valuate())
    ...
    if not proposed_trades:
        ...
        set_way_points(strategy.name, signals, '-', trade_executor.portfolio.valuate())
        return Job.WARNINGS
    ...
    trade_executor.update_portfolio_db()
    set_way_points(strategy.name, signals, processed_trades, trade_executor.portfolio.valuate())
